...
// Example makeV()
function makeV() {
    // Left diagonal leg (from top-left to bottom) - moved up by 0.05 to align with L and E
    let V2D = [
        [ vec2(-0.4,0.5), vec2(-0.2,0.5), vec2(0.0,-0.45), vec2(-0.25,-0.45) ],
        // Right diagonal leg (from top-right to bottom) - moved up by 0.05 to align with L and E
        [ vec2(0.2,0.5), vec2(0.4,0.5), vec2(0.25,-0.45), vec2(0.0,-0.45) ],
        // Small horizontal section at the bottom - moved up by 0.05 to align with L and E
        [ vec2(-0.25,-0.45), vec2(0.25,-0.45), vec2(0.25,-0.5), vec2(-0.25,-0.5) ]
    ];
...
function render(now) {
    ...
    } else if (appliedMode === "left_rotate" || (appliedMode === "none" && isAnimating)) {
        if (isAnimating) {
