...
function makeE(){
    ...
    // Front face vertices for letter E
    ...
    // Top horizontal bar
    var frontVertices2 = [
        vec4(-0.4,  0.3,  extrusionDepth, 1.0),
        vec4( 0.2,  0.3,  extrusionDepth, 1.0),
        vec4( 0.2,  0.5,  extrusionDepth, 1.0),
        vec4(-0.4,  0.5,  extrusionDepth, 1.0)
    ];
    ...
    var frontVertices3 = [
        vec4(-0.4, -0.1,  extrusionDepth, 1.0),
        vec4( 0.15, -0.1,  extrusionDepth, 1.0),
        vec4( 0.15,  0.1,  extrusionDepth, 1.0),
        vec4(-0.4,  0.1,  extrusionDepth, 1.0)
    ];
    ...
    addQuad(frontVertices1[0], frontVertices1[1], frontVertices1[2], frontVertices1[3], primaryColorVec);
    addQuad(frontVertices2[0], frontVertices2[1], frontVertices2[2], frontVertices2[3], primaryColorVec);
    addQuad(frontVertices3[0], frontVertices3[1], frontVertices3[2], frontVertices3[3], primaryColorVec);
    addQuad(frontVertices4[0], frontVertices4[1], frontVertices4[2], frontVertices4[3], primaryColorVec);

    // Back faces
    addQuad(backVertices1[3], backVertices1[2], backVertices1[1], backVertices1[0], secondaryColorVec);
    addQuad(backVertices2[3], backVertices2[2], backVertices2[1], backVertices2[0], secondaryColorVec);
    addQuad(backVertices3[3], backVertices3[2], backVertices3[1], backVertices3[0], secondaryColorVec);
    addQuad(backVertices4[3], backVertices4[2], backVertices4[1], backVertices4[0], secondaryColorVec);
    ...
    addSideFaces(
        frontVertices3[0], frontVertices3[1], frontVertices3[2], frontVertices3[3],
        backVertices3[0], backVertices3[1], backVertices3[2], backVertices3[3],
        sideColor
    );
    addSideFaces(
        frontVertices4[0], frontVertices4[1], frontVertices4[2], frontVertices4[3],
        backVertices4[0], backVertices4[1], backVertices4[2], backVertices4[3],
        sideColor
    );
...
function makeV(){
    ...
    // Front face vertices for letter V
    // Left diagonal leg (from top-left to bottom)
    var frontVertices1 = [
        vec4(-0.4,  0.5,  extrusionDepth, 1.0),   // top-left outer
        vec4(-0.2,  0.5,  extrusionDepth, 1.0),   // top-left inner
        vec4( 0.0, -0.5,  extrusionDepth, 1.0),   // bottom center point (shared)
        vec4(-0.25, -0.5, extrusionDepth, 1.0)   // bottom-left outer
    ];

    // Right diagonal leg (from top-right to bottom)
    var frontVertices2 = [
        vec4( 0.2,  0.5,  extrusionDepth, 1.0),   // top-right inner
        vec4( 0.4,  0.5,  extrusionDepth, 1.0),   // top-right outer
        vec4( 0.25, -0.5, extrusionDepth, 1.0),   // bottom-right outer
        vec4( 0.0, -0.5,  extrusionDepth, 1.0)   // bottom center point (shared)
    ];
    ...
    var backVertices1 = [
        vec4(-0.4,  0.5, -extrusionDepth, 1.0),
        vec4(-0.2,  0.5, -extrusionDepth, 1.0),
        vec4( 0.0, -0.5, -extrusionDepth, 1.0),
        vec4(-0.25, -0.5, -extrusionDepth, 1.0)
    ];
    ...
    // Small black horizontal section at the very bottom
    var frontVertices3 = [
        vec4(-0.25, -0.5,  extrusionDepth, 1.0),   // bottom-left outer
        vec4( 0.25, -0.5,  extrusionDepth, 1.0),   // bottom-right outer
        vec4( 0.25, -0.55, extrusionDepth, 1.0),   // bottom-right (extended down)
        vec4(-0.25, -0.55, extrusionDepth, 1.0)   // bottom-left (extended down)
    ];

    var backVertices3 = [
        vec4(-0.25, -0.5, -extrusionDepth, 1.0),
        vec4( 0.25, -0.5, -extrusionDepth, 1.0),
        vec4( 0.25, -0.55, -extrusionDepth, 1.0),
        vec4(-0.25, -0.55, -extrusionDepth, 1.0)
    ];
    ...
    addQuad(frontVertices1[0], frontVertices1[1], frontVertices1[2], frontVertices1[3], primaryColorVec);
    addQuad(frontVertices2[0], frontVertices2[1], frontVertices2[2], frontVertices2[3], primaryColorVec);
    // Small section at bottom (same color as other parts)
    addQuad(frontVertices3[0], frontVertices3[1], frontVertices3[2], frontVertices3[3], primaryColorVec);
    ...
    // Small section side faces (same color as other parts)
    addSideFaces(
        frontVertices3[0], frontVertices3[1], frontVertices3[2], frontVertices3[3],
        backVertices3[0], backVertices3[1], backVertices3[2], backVertices3[3],
        sideColor
...
// Helper function to apply X offset to a vertex
function offsetVertex(vertex, xOffset) {
    return vec4(vertex[0] + xOffset, vertex[1], vertex[2], vertex[3]);
}

// Helper function to apply offset to an array of vertices
function offsetVertices(vertices, xOffset) {
    var offsetVertices = [];
    for (var i = 0; i < vertices.length; i++) {
        offsetVertices.push(offsetVertex(vertices[i], xOffset));
    }
    return offsetVertices;
}

// Function to route to the appropriate letter-making function
function makeLetter() {
    points = [];
    colors = [];
    ...
    for (var i = 0; i < text.length; i++) {
        ...
        // Temporary arrays to store letter geometry
        var tempPoints = [];
        var tempColors = [];
        
        // Generate letter geometry
        switch(letter) {
            case 'L':
                generateL(tempPoints, tempColors);
                break;
            case 'E':
                generateE(tempPoints, tempColors);
                break;
            case 'V':
                generateV(tempPoints, tempColors);
                break;
            default:
                generateL(tempPoints, tempColors); // Default to L if letter not supported
                break;
        }
        
        // Apply offset to all points
        for (var j = 0; j < tempPoints.length; j++) {
            points.push(offsetVertex(tempPoints[j], currentXOffset));
            colors.push(tempColors[j]);
        }
    ...
    // Create buffers once for all letters
    var posBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, posBuffer);
    gl.bufferData(gl.ARRAY_BUFFER, flatten(points), gl.STATIC_DRAW);
    var vPosition = gl.getAttribLocation(program, "vPosition");
...
// Generate functions that add to arrays instead of creating buffers
function generateL(pointsArray, colorsArray) {
    // Front face vertices
    var frontVertices1 = [
        vec4(-0.4, -0.5,  extrusionDepth, 1.0),
        vec4(-0.2, -0.5,  extrusionDepth, 1.0),
...
function generateE(pointsArray, colorsArray) {
    // Front face vertices for letter E
    // Left vertical bar
    var frontVertices1 = [
        vec4(-0.4, -0.5,  extrusionDepth, 1.0),
    ...
    var frontVertices2 = [
        vec4(-0.4,  0.3,  extrusionDepth, 1.0),
        vec4( 0.15, 0.3,  extrusionDepth, 1.0),
        vec4( 0.15, 0.5,  extrusionDepth, 1.0),
        vec4(-0.4,  0.5,  extrusionDepth, 1.0)
    ];
...
function generateV(pointsArray, colorsArray) {
    // Front face vertices for letter V
    // Left diagonal leg (from top-left to bottom)
    var frontVertices1 = [
        vec4(-0.4,  0.5,  extrusionDepth, 1.0),   // top-left outer
